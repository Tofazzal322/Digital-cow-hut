...
export const getSingleSeller = async (sellerId: string): Promise<ISeller | null> => {
  try {
    const seller = await sellerValidationSchema.findById(sellerId);
    return seller;
  } catch (error) {
    throw error;
  }
...
export const getAllSellers = async (): Promise<ISeller[]> => {
  try {
    const sellers = await sellerValidationSchema.find();
    return sellers;
  } catch (error) {
    throw error;
  }
...
export const createSeller = async (
  ...
  try {
    const newSeller = await sellerValidationSchema.create(sellerData);
    return newSeller;
  } catch (error) {
    throw error;
  }
